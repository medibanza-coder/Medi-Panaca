...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 overflow-hidden">
      <header className="bg-white border-b border-slate-200 px-8 h-20 sticky top-0 z-50 flex justify-between items-center shadow-sm shrink-0">
      ...
      <main className="flex-1 w-full px-8 py-8 overflow-hidden flex flex-col relative bg-slate-50/50">
        {appState === 'IDLE' && (
          <div className="flex-1 flex flex-col items-center justify-center text-center animate-in fade-in duration-700">
            ...
            <div className="grid md:grid-cols-2 gap-10 w-full max-w-5xl">
              <label className="relative group bg-white p-16 rounded-[3.5rem] border-4 border-white shadow-2xl hover:border-blue-500 cursor-pointer transition-all flex flex-col items-center ring-1 ring-slate-200/50">
              ...
              <button onClick={() => setAppState('LIST')} className="bg-white p-16 rounded-[3.5rem] border-4 border-white shadow-2xl hover:border-slate-300 transition-all flex flex-col items-center group ring-1 ring-slate-200/50">
                <div className="w-20 h-20 bg-slate-50 text-slate-600 rounded-3xl flex items-center justify-center mb-8 group-hover:scale-110 transition-transform">
        ...
        {appState === 'PROCESSING' && (
          <ProcessingOverlay current={processingProgress.current} total={processingProgress.total} />
        )}
        ...
        {error && (
          <div className="fixed bottom-12 left-1/2 -translate-x-1/2 bg-rose-600 text-white px-10 py-5 rounded-[2.5rem] shadow-2xl font-black uppercase tracking-widest text-[11px] z-[200] animate-bounce text-center max-w-lg">
